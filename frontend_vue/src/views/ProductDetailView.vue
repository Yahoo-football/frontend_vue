...
async function handleAddToCart() {
  ...
  if (!product.value.is_active) {
    message.value = 'Sorry, this product is not available'
    return
  }

  // Check if product is out of stock
  if (product.value.stock === 0) {
    message.value = 'Sorry, this product is out of stock'
    return
  }

  // Check if quantity exceeds stock
  if (quantity.value > product.value.stock) {
    message.value = `Only ${product.value.stock} items available in stock`
    return
  }
  ...
  try {
    ...
    message.value = e?.response?.data?.message || 'Failed to add to cart'
  } finally {
    adding.value = false
  }
}
  ...
  <section v-else-if="product" class="product-page">
    ...
    <div class="product-card">
      ...
      <div class="info-box">
        ...
        <p class="stock" :class="{ 'out-of-stock': product.stock === 0 || !product.is_active }">
          <span v-if="!product.is_active" class="stock-badge not-available">Not Available</span>
          <span v-else-if="product.stock === 0" class="stock-badge out">Out of Stock</span>
          <span v-else-if="product.stock <= 20" class="stock-badge low">Low Stock: {{ product.stock }}</span>
          <span v-else>Stock: {{ product.stock }}</span>
        </p>

        <label class="qty" v-if="product.stock > 0 && product.is_active">
          Quantity
        ...
        <div class="actions">
          <button
            ...
            :disabled="adding || product.stock === 0 || !product.is_active"
            @click="handleAddToCart"
          >
            {{ adding ? 'Adding...' : (!product.is_active ? 'Not Available' : (product.stock === 0 ? 'Out of Stock' : 'Add to Cart')) }}
          </button>
